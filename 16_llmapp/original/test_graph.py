...
def setup_graph():
    ...
    テスト用に新しいグラフを構築。
    """
    graph = build_graph("gpt-4o-mini", memory)
    #グラフを描画して保存
    try:
        # グラフの画像をPNGファイルとして保存
        graph_image = graph.get_graph().draw_mermaid_png()
        with open("test_graph_structure.png", "wb") as f:
            f.write(graph_image)
        print("グラフ画像を test_graph_structure.png として保存しました")
    except Exception as e:
        print(f"グラフ画像の保存に失敗しました: {e}")
    return graph
